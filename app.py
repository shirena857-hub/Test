#!/usr/bin/env python3
"""
Ultimate Social Media Downloader Bot – NO SIZE LIMIT, NO COMPRESSION
...
class Config:
    ...
    # Telegram's hard limit is 50MB, but we don't check it.
    # We'll just attempt to send and let Telegram reject if too large.
...
<b>Note:</b> Telegram has a 50MB file size limit. If your video is larger, it may fail to send.
...
• If file >50MB, Telegram may reject it – try lower quality.
...
async def send_downloaded_file(query, context, info, url, user_id, user_name, platform, ydl):
    ...
    try:
        ...
        await query.message.reply_text("❌ Failed to send file. This might be due to Telegram's 50MB size limit. Try a lower quality.")
        return
