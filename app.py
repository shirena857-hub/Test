#!/usr/bin/env python3
"""
Ultimate Social Media Downloader Bot – YouTube Fixed (6 Methods)
...
class Config:
    ...
    # Base yt-dlp options (will be extended per platform)
    YDL_OPTIONS = {
        'quiet': True,
        'no_warnings': True,
        'ignoreerrors': False,
...
class StorageManager:
    ...
    def log_download(self, user_id, user_name, platform, url, filename, file_path):
        ...
        cursor.execute('''
            INSERT INTO downloads (user_id, user_name, platform, url, filename, file_path, file_size, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'downloaded')
        ''', (user_id, user_name, platform, url, filename, file_path, file_size))
        download_id = cursor.lastrowid
        today = datetime.now().date()
        cursor.execute('INSERT OR IGNORE INTO user_stats (user_id, user_name) VALUES (?, ?)', (user_id, user_name))
        cursor.execute('''
            UPDATE user_stats SET total_downloads = total_downloads + 1,
                downloads_today = CASE WHEN last_download_date = DATE(?) THEN downloads_today + 1 ELSE 1 END,
                last_download_date = DATE(?) WHERE user_id = ?
        ''', (today, today, user_id))
        conn.commit()
        conn.close()
        return download_id
    ...
    def mark_as_sent(self, download_id):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('UPDATE downloads SET status = 'sent', sent_time = CURRENT_TIMESTAMP WHERE id = ?', (download_id,))
        conn.commit()
        conn.close()
    ...
    def cleanup_old_files(self, hours_old=None):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT id, file_path FROM downloads WHERE download_time < ? AND deleted = 0', (cutoff,))
        deleted = 0
    ...
    def get_user_stats(self, user_id):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT total_downloads, downloads_today, joined_date FROM user_stats WHERE user_id = ?', (user_id,))
        row = cursor.fetchone()
        conn.close()
        if row:
...
class DownloadManager:
    ...
    def get_ydl_options(self, choice: str, platform: str) -> dict:
        ...
        if platform == "YouTube":
            # We'll handle extractor_args per method later
            pass
        ...
        if choice == "format_audio":
            ...
            opts['postprocessors'] = [p for p in opts.get('postprocessors', []) if p.get('key') != 'FFmpegVideoConvertor']
        return opts
    
    # ---------- TikTok Handler (unchanged) ----------
    ...
    async def handle_youtube(self, query, context, url, choice, user_id, user_name):
        ...
        for m in methods:
            try:
                ...
                opts.update(m['opts'])
                # Add extra headers to mimic browser
                opts['http_headers']['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
...
<b>Supported:</b> YouTube (1080p), Instagram, TikTok, Twitter, Facebook, Reddit, Pinterest (images/videos), and more.

<b>Note:</b> For YouTube, you must add a <code>cookies.txt</code> file to the bot folder (instructions in repo).
...
<b>Tips:</b>
• For YouTube 1080p, add cookies.txt (see instructions below).
• Pinterest images auto-detected.
• No file size limits – large files may take longer to upload.

<b>📌 YouTube cookies.txt instructions:</b>
1. Install "Get cookies.txt" extension in Chrome.
2. Log in to YouTube in your browser.
3. Click the extension icon and export cookies.
4. Save the file as <code>cookies.txt</code> in the bot's root folder.
5. Redeploy the bot.
...
async def send_downloaded_file(query, context, info, url, user_id, user_name, platform, ydl):
    ...
    try:
        ...
        await query.message.reply_text("❌ Failed to send file. This may be due to a Telegram limitation.")
        return
...
async def show_error(query, error_msg, platform, url):
    ...
    if "Sign in" in error_msg:
        error_msg = "YouTube requires authentication. Add cookies.txt to bot folder."
...
def main():
    ...
    # Check if cookies.txt exists for YouTube
    if os.path.exists('cookies.txt'):
        console.print("[green]✓ cookies.txt found (YouTube ready)[/green]")
    else:
        console.print("[yellow]⚠ cookies.txt not found – YouTube may fail. Add it for reliable 1080p downloads.[/yellow]")
